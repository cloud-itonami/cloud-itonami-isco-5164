...
- `src/pet_care/store.cljk` — `Store` protocol +
  `MemStore`: registered animals, committed records, an append-only
  audit ledger.
- `src/pet_care/advisor.cljk` — `Advisor` protocol;
...
- `src/pet_care/actor.cljk` — `build-graph`, `run-request!`,
