(ns pet-care.advisor
  "PetCareAdvisor — proposes a pet-grooming-and-animal-care operation
  (groom-support step, monitor entry, operating near an
  anxious/aggressive animal, sedation procedure) for a registered
  animal. The advisor is swappable: `mock-advisor` (deterministic,
  default in dev/tests/CI) or `llm-advisor` (wraps a real
  `langchain.model/ChatModel`). Either way the advisor ONLY produces a
  PROPOSAL — it never writes to the store and has no notion of animal
  provenance or aggressive-animal/sedation risk; `pet-care.governor`
  is the independent system that decides whether the proposal may
  proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :groom-support|:monitor|:operate-near-anxious-aggressive-animal|:sedation-procedure
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for animal " (:animal-id request))})
...
(def ^:private system-prompt
  "You are a pet-grooming-and-animal-care advisor. Given a
   groom-support or monitor operation request, propose an :op, an
