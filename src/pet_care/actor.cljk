(ns pet-care.actor
  "PetCareActor — the ISCO-08 5164 independent pet-grooming-and-
  animal-care actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  groom-support/monitor operation request (intake → advise → govern →
  ...
  The unconditional invariant: the PetCareAdvisor can never directly
  commit a record or dispatch a robot action the PetCareGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [pet-care.advisor :as advisor]
            [pet-care.governor :as governor]
            [pet-care.store :as store]))

(defn build-graph
  "Build a compiled PetCareActor graph. `store` implements
  `pet-care.store/Store`. `advisor` implements
  `pet-care.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:animal-id (:animal-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
